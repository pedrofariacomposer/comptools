...
def make_guide_score(timesigs, first_bar=1):

    """Given a list of time signatures, returns a string
    to be used in LilyPond, to create the sequence of time signatures
    for a new piece.
    """
    result = []
    i = 0
    while i < len(timesigs):
        current = timesigs[i]
        frac = str(current[0]) + "/" + str(current[1])
        ts = f"\\time {frac}"
        s = f"s1 * {frac} % bar {first_bar}"
        if i == 0:
            result.append(ts)
            result.append(s)
        else:
            lastsig = timesigs[i-1]
            if lastsig != current:
                result.append(ts)
                result.append(s)
            else:
...
def make_guide_staff(
    timesigs: List,
    voiceName: str ="Guide Voice",
    staffName: str = "Global Context"
) -> abjad.Staff:

    """Given a list of time signatures, creates a guide staff with the
    time signatures
    """

    voice = abjad.Voice(name=voiceName)
    for timeSig in timesigs:
        sig = abjad.TimeSignature(timeSig)
        rest = "s1 * " + str(timeSig[0]) + "/" + str(timeSig[1])
        voice.append(rest)
        comp = abjad.select.leaf(voice, -1)
        abjad.attach(sig, comp)
    staff = abjad.Staff([voice], name=staffName)
    literal = abjad.LilyPondLiteral("\\numericTimeSignature")
    abjad.attach(literal, abjad.select.leaf(voice,0))
